...
const methodConfig: Record<PaymentMethod, { label: string; shortLabel: string; icon: typeof Banknote; currency: "USD" | "BS" }> = {
  ...
  card_bs: { label: "TARJETA (BS)", shortLabel: "TARJETA BS", icon: CreditCard, currency: "BS" },
  transfer_bs: { label: "TRANSF. (BS)", shortLabel: "TRANSFERENCIA BS", icon: Smartphone, currency: "BS" },
};

export function PaymentModal({ open, onClose, totalUsd, onConfirm }: PaymentModalProps) {
  ...
  const totalPaidUsd = useMemo(
    () => payments.reduce((sum, p) => sum + p.amountUsd, 0),
    [payments]
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-4 grid grid-cols-1 md:grid-cols-[1fr,1fr] rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-card p-6 flex flex-col gap-5">
          ...
          <div className="border border-border rounded-2xl p-4 space-y-3">
            ...
            <Input
              ...
              className="border-none shadow-none text-3xl font-black h-auto py-1 px-0 focus-visible:ring-0 placeholder:text-muted-foreground/40"
              min="0"
              step="0.01"
            />
        ...
        <div className="bg-muted/30 p-6 flex flex-col gap-4 border-l border-border">
          {/* Total */}
          <div>
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-3 bg-card rounded-xl p-3 border border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {p.reference && (
                          <p className="text-[10px] text-primary font-medium">REF: {p.reference}</p>
                        )}
                      </div>
